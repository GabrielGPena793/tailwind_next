...
const fileItem = tv({
  slots: {
    container:
      'group flex items-center gap-4 rounded-lg border border-zinc-200 p-4',
  ...
  variants: {
    state: {
      complete: {
        container: 'border-violet-500 dark:border-zinc-300/30',
      },
      error: {
        container:
    ...
    <div className={container()}>
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <button
            type="button"
            className="text-sm font-semibold text-error-700 hover:text-error-900 dark:text-error-400 hover:dark:text-error-300"
          >
            Try again
          </button>
        </div>
